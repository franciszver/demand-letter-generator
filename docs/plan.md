...
**Frontend:** React (TypeScript) with Vite build tool, Tailwind CSS for styling, React Context API for state  
**Backend:** Node.js/Express (TypeScript) REST API with serverless-http wrapper for Lambda compatibility  
**Database:** PostgreSQL for metadata persistence (Knex.js migrations)  
**Storage:** AWS S3 for document storage (full text content), PostgreSQL for metadata  
**AI:** OpenRouter API with GPT-4o model  
**Compute:** AWS Lambda functions (serverless-first, Express-compatible)  
**Package Manager:** npm  
**Deployment:** AWS SAM for infrastructure, Local development + AWS (S3/CloudFront frontend, Lambda backend)
...
- Initialize package.json files with npm
- Create `.env.example` files for configuration
- Set up shared TypeScript types between frontend and backend

### 1.2 Database Setup
- Design PostgreSQL schema (single-firm architecture):
  - `users` table (authentication and user management)
  - `templates` table (demand letter templates - global to firm)
  - `documents` table (uploaded source documents metadata, S3 key references)
  - `draft_letters` table (generated and refined letters, S3 key for full content)
  - `sessions` table (for real-time collaboration tracking)
- Store full document text and letter content in S3, keep metadata/summaries in PostgreSQL
...
- Extract text content and store full text in S3 (processed/ bucket)
- Store document metadata and S3 key in PostgreSQL
- Return document ID and extracted content summary
...
- Apply firm-specific templates when provided (plain text with {{variable_name}} placeholders)
...
- Apply proper formatting (1-inch margins, Times New Roman, proper spacing)
- Include citations and references
- Generate downloadable .docx file

### 2.2 API Handlers (Lambda Functions)
**File:** `backend/src/handlers/upload.ts`
- POST /api/upload - Validate file type/size (50MB limit), process document, store metadata in PostgreSQL
...
- POST /api/templates - Create new template (plain text with variable placeholders like {{client_name}}, {{date}}, etc.)
...
- Track document sessions and user presence in PostgreSQL
- Broadcast changes to all connected clients
- Implement basic change tracking (who made changes, when)
- Show active users viewing/editing documents
...
- Implement JWT-based authentication (simple auth for MVP)
- Add middleware for route protection
- Encrypt sensitive data at rest
- Use presigned URLs for S3 access
- Implement rate limiting
- Add audit logging for compliance
- Environment-based error handling (user-friendly in production, detailed in development)
...
- Basic change tracking display (shows who made changes, timestamps)
- Active user presence indicators (who's viewing/editing)
- Save draft functionality

**File:** `frontend/src/components/TemplateManager.tsx`
- List templates in a table/cards
- Create/edit template form with text editor (plain text with variable placeholders like {{client_name}}, {{date}})
- Template variable/placeholder support and validation
...
- Environment-aware error messages (user-friendly in production)
...
- Use S3 for large text storage to keep PostgreSQL queries fast
...
- Set up local environment variables (.env files)
- Create development scripts
- Configure CORS for localhost development
- Document local setup process
- Set up basic unit tests for critical functions (AI services, document processing)
...
- **Word Export:** docx library with standard professional formatting
...
- **S3 Buckets:** `demand-letter-generator-{env}-{purpose}`
- **Initial Admin:** Environment variable approach
